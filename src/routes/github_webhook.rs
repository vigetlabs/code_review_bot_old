...
fn handle_pull_request_opened(
    ...
    let happy_path = future::result(if json.pull_request.draft {
        Err(error::ErrorBadRequest("Ignoring Draft PR"))
    } else {
        Ok(())
    })
    .and_then(move |_| {
...
pub fn pull_request(req: actix_web::HttpRequest<AppConfig>) -> FutureResponse<HttpResponse> {
    ...
    Json::<PullRequestEvent>::extract(&req)
        ...
        .and_then(move |(state, json)| match json.action {
            ...
            _ => future::Either::B(future::err(error::ErrorNotFound(format!("Unhandled PR Action: {:?}", json.action)))),
        })
        .responder()
}
